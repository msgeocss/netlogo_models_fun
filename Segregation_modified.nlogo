...
turtles-own [
  ...
  my-%-similar-wanted  ;; what percent they want of others nearby
  my-color-%-similar-wanted
  happy?2
...
to setup
  ...
  ask turtles
      [ if distribution = "uniform" [set my-%-similar-wanted %-similar-wanted]
       if distribution = "random-normal" [set my-%-similar-wanted random-normal %-similar-wanted 2]
      ]


  ask turtles with [color = red]
      [ if distribution = "uniform" [set my-color-%-similar-wanted red-%-similar-wanted]
       if distribution = "random-normal" [set my-color-%-similar-wanted random-normal red-%-similar-wanted 2]
      ]


  ask turtles with [color = green]
      [ if distribution = "uniform" [set my-color-%-similar-wanted green-%-similar-wanted]
        if distribution = "random-normal" [set my-color-%-similar-wanted random-normal green-%-similar-wanted 2]
      ]
...
to go
  ifelse vary-by-color
     [if all? turtles [ happy?2 ] [ stop ] ]
...
to move-unhappy-turtles
  ifelse vary-by-color
    [ask turtles with [ not happy?2 ] [ find-new-spot ]]
...
to update-turtles
  ask turtles [
    ...
    set happy?2 similar-nearby >= (my-color-%-similar-wanted * total-nearby / 100)
    ...
    if visualization = "square-x" [
      ifelse vary-by-color = "on"
         [ ifelse happy?2 [ set shape "square" ] [ set shape "square-x" ]]
         [ ifelse happy? [ set shape "square" ] [ set shape "square-x" ]]
        ]

    if visualization = "mismatch" [
      ifelse happy? != happy?2 [set shape "circle"] [set shape "square"]
      ]
  ]
end
...
to update-globals
  ...
  set percent-mismatch (count turtles with [ not happy? = happy?2]) / (count turtles) * 100
  set percent-unhappy2 (count turtles with [ not happy?2 ]) / (count turtles) * 100
...
"" ""
PENS
"not happy?" 1.0 0 -817084 true "" "plot count turtles with [not happy?]"
"not happy?2" 1.0 0 -8431303 true "" "plot count turtles with [not happy?2]"
"mismatch" 1.0 0 -14835848 true "" "plot count turtles with [happy? != happy?2]"
...
378
num-mismatch
count turtles with [happy? != happy?2]
17
1
11
...
378
num-unhappy?2
count turtles with [not happy?2]
17
1
11
...
"" ""
PENS
"Green" 1.0 0 -14439633 true "" "plot count turtles with [color = green and happy?2] / count turtles with [color = green] * 100"
"Red" 1.0 0 -5298144 true "" "plot count turtles with [color = red and happy?2] / count turtles with [color = red] * 100 "
...
1083
39
Applied to total pop regardless of color
11
0.0
1
...
This Segregation model from the model library of Netlogo was modified to compare results
when green and red are allowed to have different preferences, and also to compare impact of allowing some heterogeneity in the preferences versus everyone having the same.
